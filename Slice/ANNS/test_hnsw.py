...
model_size = os.path.getsize(model_file) / math.pow(2, 20)
model_size += (num_lbls * num_ft * 4) / math.pow(2, 20)
...
for i in range(num_out_threads):
    Process(target=write_knn_out, args=(out_dir, write_dist, num_lbls,
                                        nbrs[i*batch_size:min((i+1)*batch_size, len(nbrs))], i, metric_space)).start()
