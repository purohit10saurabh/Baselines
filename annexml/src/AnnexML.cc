...
int AnnexML::Init(const AnnexMLParameter &param, bool load_model) {
  ...
  if (load_model) {
    ...
    fprintf(stdout, "Total: %.2f MB => labels: %.2f MB, partitioning: %.2f MB, embedding: %.2f MB (matrix: %.2f MB, embed: %.2f MB) \n", total_size / mb, labels_size / mb, partitioning_vec_size / mb, embedding_vec_size / mb, w_size / mb, e_size / mb);
    ...
    if (param_.pred_type() > 0) {
      ...
      fprintf(stdout, "num_edge: %d, search_eps: %f\n", param_.num_edge(), param_.search_eps());
      fprintf(stdout, "Build SearchIndex...");
...
#pragma omp parallel for schedule(dynamic)
      ...
      fprintf(stdout, "\rBuild SearchIndex done! (%.2f sec, %.2f MB)\n", elapsed, index_size / mb);
    }

  } else {

    if (! param_.has_train_file()) { fprintf(stdout, "train_file is not specified\n"); return -1; }
  }
...
int AnnexML::Train() {
  ...
  if (ret != 1) { fprintf(stdout, "Data load error!: '%s'\n", param_.train_file().c_str()); return -1; }

  size_t num_data = data_vec.size();
  fprintf(stdout, "Load Train Data: #data=%lu #feature=%d #label=%d\n", num_data, max_fid, max_lid);
  fflush(stdout);
  double start = Utils::GetTimeOfDaySec();
  ...
  for (size_t i = 0; i < num_learner; ++i) {
    ...
    fprintf(stdout, "Learner: %2lu (#data=%lu)", i, data_vec.size());
    for (size_t j = 0; j < total_prec_vec.size(); ++j) {
      fprintf(stdout, ", P@%lu: %.4f", j+1, total_prec_vec[j] / data_vec.size());
    }
    fprintf(stdout, "\n");
  }
  double elapsed = Utils::GetTimeOfDaySec() - start;
...
int AnnexML::Predict() const {
  ...
  if (ret != 1) { fprintf(stdout, "Fail to open predict_file: '%s'\n", param_.predict_file().c_str()); return -1; }

  size_t num_data = data_vec.size();
  fprintf(stdout, "Load Predict Data: #data=%lu #feature=%d #label=%d\n", num_data, max_fid, max_lid);
...
#pragma omp parallel for schedule(dynamic)
  ...
  fprintf(stdout, "Elapsed: %.4f sec, %.4f msec/sample (* %lu threads = %.4f msec/sample)\n", elapsed, elapsed_per_sample, num_thread, num_thread * elapsed_per_sample);
  fprintf(stdout, "Partitioning: %.4f sec, Embedding: %.4f sec, Aggregating: %.4f sec\n", c_elapsed, e_elapsed, a_elapsed);
...
int AnnexML::LearnPartitioning(const std::vector<std::vector<std::pair<int, float> > > &data_vec,
  ...
  if (verbose == 0) {
    fprintf(stdout, "\rPartitioning progress: %6.2f%% (#task=%lu/%lu)", 0.0f, num_done_task, num_learner);
  }
...
#pragma omp parallel for schedule(dynamic)
  for (size_t i = 0; i < num_learner; ++i) {
    ...
    if (verbose == 0) {
      ...
      fprintf(stdout, "\rPartitioning progress: %6.2f%% (#task=%lu/%lu)", progress, num_done_task, num_learner);
      omp_unset_lock(&lock);
    }
  }
  if (verbose == 0) { fprintf(stdout, "\rPartitioning done!                                    \n"); }
...
int AnnexML::LearnEmbedding(const std::vector<std::vector<std::pair<int, float> > > &data_vec,
  ...
  size_t num_done_task = 0;
  if (verbose == 0) {
    fprintf(stdout, "\rEmbedding progress: %6.2f%% (#task=%lu/%lu)", 0.0f, num_done_task, etask_vec.size());
  }
...
#pragma omp parallel for schedule(dynamic)
  for (size_t i = 0; i < etask_vec.size(); ++i) {
    ...
    if (verbose == 0) {
      ...
      fprintf(stdout, "\rEmbedding progress: %6.2f%% (#task=%lu/%lu)", progress, num_done_task, etask_vec.size());
      omp_unset_lock(&lock);
    }
  }
  if (verbose == 0) { fprintf(stdout, "\rEmbedding done!                                     \n"); }
