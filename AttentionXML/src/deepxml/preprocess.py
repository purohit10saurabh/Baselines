...
def tokenize(sentence: str, sep='/SEP/'):
    # We add a /SEP/ symbol between titles and descriptions such as Amazon datasets.
...
def main(text_path, tokenized_path, label_path, vocab_path, emb_path, w2v_model, vocab_size, max_len):
    if tokenized_path is not None:
        ...
        with open(text_path, encoding='latin1') as fp, open(tokenized_path, 'w', encoding='latin1') as fout:
    ...
    if not os.path.exists(vocab_path):
        ...
        with open(text_path, encoding='latin1') as fp:
    ...
    texts, labels = get_data(text_path, label_path, False, max_len, vocab)
