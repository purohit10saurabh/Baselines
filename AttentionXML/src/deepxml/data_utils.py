...
__all__ = ['build_vocab', 'get_data', 'truncate_text', 'get_word_emb', 'get_mlb', 'get_sparse_feature', 'output_res']
...
def get_data(text_file, label_file=None, binary=True, max_len=None, vocab=None, unknown='<UNK>'):
    if binary:
        texts, labels = np.load(text_file, allow_pickle=True), np.load(label_file, allow_pickle=True) if label_file is not None else None
    else:
        with open(text_file, encoding='latin1') as fp:
            texts = np.asarray([[vocab.get(word, vocab[unknown]) for word in text.split()]
                               for text in fp])
        labels = None
        if label_file is not None:
            with open(label_file, encoding='latin1') as fp:
                labels = np.asarray([[label for label in line.split()] for line in fp])
    return truncate_text(texts, max_len), labels


def truncate_text(texts, max_len=500):
    if max_len is None:
        return texts
    texts = np.asarray([list(x[:max_len]) + [0] * (max_len - len(x)) for x in texts])
    texts[(texts == 0).all(axis=1), 0] = 1
    return texts
...
def get_sparse_feature(feature_file):
    if isinstance(feature_file, tuple) or isinstance(feature_file, list):
        feature_file, label_file = feature_file
    else:
        label_file = None
    sparse_x, sparse_y = load_svmlight_file(feature_file, multilabel=True)
    sparse_y = [[str(int(yy)) for yy in y] for y in sparse_y] if label_file is None else np.load(label_file)
    return normalize(sparse_x), sparse_y
