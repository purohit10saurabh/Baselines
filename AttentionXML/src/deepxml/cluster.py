...
def build_tree(labels_f: csr_matrix, eps: float, max_leaf: int):
    q, labels_list = deque([(np.arange(labels_f.shape[0]), normalize(labels_f))]), []
    while q:
        node_i, node_f = q.popleft()
        if len(node_i) <= max_leaf:
            labels_list.append(node_i)
        else:
            q += list(split_node(node_i, node_f, eps))
    assert sum(len(labels) for labels in labels_list) == labels_f.shape[0]
    return np.asarray(labels_list)


def build_tree_by_level(sparse_data, mlb, eps: float, max_leaf: int, levels: list, groups_path):
    os.makedirs(os.path.split(groups_path)[0], exist_ok=True)
    logger.info('Clustering')
    sparse_x, sparse_labels = get_sparse_feature(sparse_data)
