...
__all__ = ['Embedding', 'LSTMEncoder', 'MLAttention', 'AttentionWeights', 'FastMLAttention', 'MLLinear', 'Empty']


def get_linear(in_features, out_features, bias=True, gain=1):
    linear = nn.Linear(in_features, out_features, bias)
    nn.init.xavier_uniform_(linear.weight, gain)
    return linear
...
class Embedding(nn.Module):
    ...
    """
    def __init__(self, vocab_size=None, emb_size=None, emb_init=None, emb_trainable=True, dropout=0.2,
                 retain_grad=False):
        ...
        self.emb = nn.Embedding(vocab_size, emb_size, padding_idx=0, sparse=True,
        ...
        self.emb_out, self.retain_grad = {}, retain_grad

    def forward(self, inputs, perturbation=None):
        emb_out = self.emb(inputs)
        if perturbation is None:
            emb_out = self.dropout(emb_out)
            if self.retain_grad and self.training:
                if not self.emb.weight.requires_grad:
                    emb_out.requires_grad = True
                emb_out.retain_grad()
                self.emb_out[inputs.device] = emb_out
        else:
            emb_out = emb_out.masked_fill(self.emb_out[inputs.device] == 0.0, 0.0) + perturbation
...
class LSTMEncoder(nn.Module):
    ...
    def forward(self, inputs, lengths, **kwargs):
        ...
        # return self.dropout(self.lstm(inputs)[0])
