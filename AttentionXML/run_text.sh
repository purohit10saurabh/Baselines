...
PREPROS='../tools/data/preProcess.perl'
...
prep_data(){
	get_data_text(){
		prefix=$1
		awk -F: '{ st = index($0,"->");print substr($0,st+2)}' "${raw_data_dir}/${prefix}_map.txt" | tr '_' ' '> "${data_dir}/${prefix}_tit"
		awk -F: '{ st = index($0,"->");print substr($0,st+2)}' "${raw_data_dir}/${prefix}_raw_text.txt" | tr '_' ' '| $PREPROS  > "${data_dir}/${prefix}_txt"
        paste -d '\t' "${data_dir}/${prefix}_tit" "${data_dir}/${prefix}_txt" | awk -F '\t' '{print $1,"/SEP/",$2}' > "${data_dir}/${prefix}_full"
        perl $CONVERT "${data_dir}/${prefix}.txt" "$data_dir/${prefix}_X_X.txt" "$data_dir/${prefix}_X_Y.txt"
		args="${data_dir}/${prefix}_raw_ids.txt \
				${data_dir}/${prefix}_full \
				${data_dir}/${prefix}_X_Y.txt \
				${data_dir} \
				${prefix}"
		python $FETCHDATA $args
		rm -rf "${data_dir}/${prefix}_X"*
        rm -rf "${data_dir}/${prefix}_full"
        rm -rf "${data_dir}/${prefix}_tit"
        rm -rf "${data_dir}/${prefix}_txt"
	}
    
    awk -F: '{ st = index($0,"->");print substr($0,st+2)}' "${raw_data_dir}/train_map.txt" | tr '_' ' ' > "${data_dir}/raw_tits.txt"
    awk -F: '{ st = index($0,"->");print substr($0,st+2)}' "${raw_data_dir}/train_raw_text.txt" | tr '_' ' '| $PREPROS  > "${data_dir}/raw_txt.txt"
    paste -d '\t' "${data_dir}/raw_tits.txt" "${data_dir}/raw_txt.txt" | awk -F '\t' '{print $1,"/SEP/",$2}' > "${data_dir}/raw_full.txt"
	get_data_text "train"
	get_data_text "test"
