...
def load_data_and_labels(data,label_size):
...
def load_data(data_path, max_length=500, vocab_size=50000,labels=5000):
    # Load and preprocess data
    with open(os.path.join(data_path), 'rb') as fin:
        [train, test, vocab, catgy, valid_train_instances, valid_test_instances] = pickle.load(fin)
    # dirty trick to prevent errors happen when test is empty
    if len(test) == 0:
        test[:5] = train[:5]

    trn_sents, Y_trn = load_data_and_labels(train,labels)
    tst_sents, Y_tst = load_data_and_labels(test,labels)
    ...
    return X_trn, Y_trn, X_tst, Y_tst, vocabulary, vocabulary_inv, valid_train_instances, valid_test_instances
