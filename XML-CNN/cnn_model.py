...
class CNN_model(object):
    ...
    def add_data(self, X_trn, Y_trn,labels):
    ...
    def add_pretrain(self, vocabulary, vocabulary_inv,model_dir):
        print('model_variaton:', self.model_variation)
        if self.model_variation=='pretrain':
            embedding_weights = load_word2vec(self.pretrain_type, vocabulary_inv, model_dir, self.embedding_dim)
    ...
    def build_predict(self, epoch):
        ...
        if self.model_variation == 'pretrain':
            ...
            raise NotImplementedError('Unknown model_variation!')
        l_embed_dropout = lasagne.layers.DropoutLayer(l_embed, p=0.0)
        ...
        # Final hidden layer
        l_hidden = lasagne.layers.DenseLayer(l_conv_final, num_units=self.hidden_dims, nonlinearity=lasagne.nonlinearities.rectify)
        l_hidden_dropout = lasagne.layers.DropoutLayer(l_hidden, p=0.0)
        l_y = lasagne.layers.DenseLayer(l_hidden_dropout, num_units=self.output_dim, nonlinearity=lasagne.nonlinearities.sigmoid)
        self.network = l_y
    ...
    def train(self):
        ...
        nr_batches = int(np.ceil(nr_trn_num / np.float32(self.batch_size)))
    ...
    def predict(self, X_tst=None, batch_size=128, top_k=5):
        nr_tst_num = X_tst.shape[0]
        nr_batches = int(np.ceil(nr_tst_num / np.float32(batch_size)))
        ...
        for batch_idx in xrange(nr_batches):
            ...
            for i in xrange(Y_pred.shape[0]):
                ...
                val_idx_list += (Y_pred[i, sorted_idx]+1e-7).tolist()
        m = nr_tst_num
        n = self.output_dim
    ...
    def show_params(self,epoch):
        ...
        print "Model size is : {}".format(total_params*4.0/(1024*1024*1024))
